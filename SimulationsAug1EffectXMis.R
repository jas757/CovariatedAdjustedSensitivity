...
make.dat = function(){
  ...
  X4 = xx[, 4]
  X5 = xx[, 5]
  prob.m =  exp(-0.1 + 0.5 * A - 1.5 *X1 + 1.5 *X2 - 0.5 * X1^2 + 0.5 * X2^2)/(1 + exp(-0.1 + 0.5 * A  - 1.5 *X1 + 1.5 *X2 - 0.5 *X1^2  + 0.5 * X2^2))
  M = rbinom(n, 1, prob.m)
  prob.y = exp(-0.1 + 0.1 * A  - 1 *X1 -1 *X2 + 1 * M - 0.5 *X1^2  + 0.5 * X2^2)/(1 + exp(-0.1 + 0.1 * A  - 1 *X1 -1 *X2 + 1 * M - 0.5 *X1^2  + 0.5 * X2^2))
  Y = rbinom(n, 1, prob.y)
...
save(data.comb, file = "ResultsNoMissMeasnMisEffModelMis2500.rda")
