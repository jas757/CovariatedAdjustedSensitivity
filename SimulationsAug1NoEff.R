...
make.dat = function(){
  ...
  X4 = xx[, 4]
  X5 = xx[, 5]
  prob.m =  exp(-0.1 + 0.5 * A - 0 *X1 + 0 *X2)/(1 + exp(-0.1 + 0.5 * A  - 0 *X1 + 0 *X2))
  M = rbinom(n, 1, prob.m)
  prob.y = exp(-0.1 + 0.1 * A  - 0 *X1 -0 *X2 + 1 * M)/(1 + exp(-0.1 + 0.1 * A  - 0 *X1 - 0 *X2 + 1 * M))
  Y = rbinom(n, 1, prob.y)
...
save(data.comb, file = "ResultsNoMissMeasnNoEff5000.rda")
